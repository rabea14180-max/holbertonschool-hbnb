...
erDiagram
    ...
    %% Many-to-many: Place <-> Amenity via join table
...
| Place ↔ Amenity | Many-to-Many | Via the `PLACE_AMENITY` join table; a place can have many amenities and an amenity can appear in many places |

## Constraints

- `REVIEW(user_id, place_id)` — **UNIQUE**: a user can only submit one review per place  
